...
class Channel:
    def __init__(self, data):
        ...
        self.tags = self._data.get("tags") #[ChannelTag(tag) for tag in channel["tags"]]
        self.live_since_datetime = self._data.get("live_since_datetime")
        self.live_until_datetime = self._data.get("live_until_datetime")
        self.ancestors = self._data.get("ancestors") #[Channel(_data) for _data in self._data.get("ancestors")]
        self.syncable_tags = self._data.get("syncable_tags") #[ChannelTag(tag) for tag in channel["syncable_tags"]]
